...
range, but in addition to the .front property which provides the elements
there are also the property:

  .min = The minimum position of a data element in coordinate space,
         Think of this as the bin minimum in other respects

  .coord = The location of the .front element in coordinate space.
  		
  .max = The maximum position of a data element in coordinate space
         Think of this as the bin maximum.
			
It is okay for .min = .coord = .max.

In addition to the data range there is a priority data range which adds
the following property:

  .priority
  
this is a comparable, preferrable an integer.

*/

module das2.algorithm;
...
*/
enum bool isPriorityRange = isDataRange!R
 ...
 *  CFunc = A function for producing coords from each element
 */

struct DataRange(RT, CT, CFunc) {
 ...
 * '''
 */
PriorityRange!(RT, ET) priorityRange(RT, ET = ElementType!RT)(
...
unittest
{
	...
	pr = packets.priorityRange((a => a[0]));

	pr5.popFront();
	assert((pr5.min == 108.0)&&(pr5.max == 112.0), "Algorithm test 1 failed");
	assert(pr5.priority == 5, "Algorithm test 2 failed"); 
}
